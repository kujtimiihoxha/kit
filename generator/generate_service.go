...
func (g *generateServiceEndpointsBase) Generate() (err error) {
	...
	for _, v := range g.serviceInterface.Methods {
		...
		//deal with const define
		vdef:= jen.Id("Ep" + v.Name + "Name").Op("=").Lit(v.Name)
		vars = append(vars, vdef)

		//deal with endpoints middlewares
		eps[jen.Id(v.Name+"Endpoint")] = jen.Id("Make" + v.Name + "Endpoint").Call(jen.Id("s"))
		l := jen.For(jen.List(jen.Id("_"), jen.Id("m")).Op(":=").Range().Id("mdw").Index(jen.Id("Ep" + v.Name + "Name")))
		l.Block(
